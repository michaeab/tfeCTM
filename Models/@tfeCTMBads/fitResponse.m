...
if (obj.dimension == 2)
    ...
    if lockAngle
        if ~isempty(p.Results.initialParams)
            vubParams.angle = p.Results.initialParams.angle;
            vlbParams.angle = p.Results.initialParams.angle;
        else
            error('Default params must be provided')
        end
    end

    % Minor axis ratio bounds
    if lockMAR
        if ~isempty(p.Results.initialParams)
            vubParams.minAxisRatio = p.Results.initialParams.minAxisRatio;
            vlbParams.minAxisRatio = p.Results.initialParams.minAxisRatio;
        else
            error('Default params must be provided')
        end
    end
